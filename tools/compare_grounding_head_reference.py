...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare grounding-head outputs against MLX reference.")
...
def main() -> int:
    ...
    for i in range(6):
        compare_arrays(
            f"pred_logits_{i:02d}",
            np.load(args.reference_prefix.with_name(f"{args.reference_prefix.name}.pred_logits_{i:02d}.npy")),
            np.load(args.candidate_prefix.with_name(f"{args.candidate_prefix.name}.pred_logits_{i:02d}.npy")),
        )
        compare_arrays(
            f"pred_boxes_{i:02d}",
            np.load(args.reference_prefix.with_name(f"{args.reference_prefix.name}.pred_boxes_{i:02d}.npy")),
            np.load(args.candidate_prefix.with_name(f"{args.candidate_prefix.name}.pred_boxes_{i:02d}.npy")),
        )
    return 0
